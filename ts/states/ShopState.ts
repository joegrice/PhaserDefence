module Game {
    export class ShopState extends Phaser.State {
        ...
        create() {
            let towers = [
                { name: "redtower", type: Models.RedTowerStats },
                { name: "greentower", type: Models.GreenTowerStats },
                { name: "smallgreentower", type: Models.SmallGreenTowerStats },
                { name: "smallyellowtower", type: Models.SmallYellowTowerStats }
            ];
            ...
            var moneyText = this.game.add.text(this.game.world.centerX, 150, "Money: " + Models.GameStats.money, { font: "25px Arial", fill: "#ffffff" });
            ...
            for (let i = 0; i < towers.length; i++) {
                ...
                btn.events.onInputDown.add(() => {
                    if (Models.GameStats.money >= 50) {
                        towers[i].type.bulletSpeed += 50;
                        Models.GameStats.money -= 50;
                        moneyText.text = "Money: " + Models.GameStats.money;
                    } else {
